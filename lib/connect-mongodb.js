 ...
 * @param {Function} afterReap Returns the list of sids to be delted back to a function for futher processing if necessary
 * @api public
 */
var MONGOSTORE = module.exports = function MongoStore(options, callback, afterReap) {
  var colAction;
  options = options || {};
      ...
      if(typeof afterReap === 'function') {
        colAction = function () {
          _collection.find({
            ...
            .toArray(function (err, sidResult) {
  ...
  if(options.url) {
    ...
    url.pathname.replace(/^\//, ''), new mongo.Server(url.hostname || _defaults.host, + url.port || _defaults.port));
  }
